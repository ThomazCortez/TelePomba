...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        $stmt = $pdo->prepare("
    SELECT id_utilizador, nome_utilizador, palavra_passe, imagem_perfil 
    FROM utilizadores 
    WHERE nome_utilizador = :username OR email = :email
");
        ...
        if ($user) {
            if (password_verify($password, $user["palavra_passe"])) {
                ...
                error_log("User image path: " . $user['imagem_perfil']);
                error_log("All user data: " . print_r($user, true));
                ...
                $_SESSION["login_error"] = "Palavra-passe incorreta";
            }
        } else {
...
// REMOVER ESTE REDIRECIONAMENTO FINAL
// header("Location: login.php"); // <- Isto estava causando o loop
// exit();
?>
<!DOCTYPE html>
<html lang="pt-PT">
<head>
    ...
    <title>Iniciar Sessão | TelePomba</title>
    <!-- Bootstrap CSS -->
